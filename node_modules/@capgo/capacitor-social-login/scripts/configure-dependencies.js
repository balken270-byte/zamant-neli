...
function applyDependencyReplacements(content, replacements) {
...
function getPodspecReplacements(providerConfig) {
...
function getPackageSwiftReplacements(providerConfig) {
    const swiftCommentPrefix = '(?:\\/\\/[ \\t]*)*';
    const swiftTrailingComment = '(?:[ \\t]*\\/\\/[^\\r\\n]*)?';
    return [
        {
            old: new RegExp(`${swiftCommentPrefix}\\.package\\(url:\\s*"https:\\/\\/github\\.com\\/facebook\\/facebook-ios-sdk\\.git",\\s*\\.upToNextMajor\\(from:\\s*"[^"]+"\\)\\),${swiftTrailingComment}`),
            new: providerConfig.facebook === 'implementation'
        ...
        {
            old: new RegExp(`${swiftCommentPrefix}\\.package\\(url:\\s*"https:\\/\\/github\\.com\\/google\\/GoogleSignIn-iOS\\.git",\\s*\\.upToNextMajor\\(from:\\s*"[^"]+"\\)\\),${swiftTrailingComment}`),
            new: providerConfig.google === 'implementation'
        ...
        {
            old: new RegExp(`${swiftCommentPrefix}\\.package\\(url:\\s*"https:\\/\\/github\\.com\\/Alamofire\\/Alamofire\\.git",\\s*\\.upToNextMajor\\(from:\\s*"[^"]+"\\)\\)${swiftTrailingComment}`),
            new: providerConfig.apple === 'implementation'
        ...
        {
            old: new RegExp(`${swiftCommentPrefix}\\.product\\(name:\\s*"FacebookCore",\\s*package:\\s*"facebook-ios-sdk"\\),${swiftTrailingComment}`),
            new: providerConfig.facebook === 'implementation'
                ? `.product(name: "FacebookCore", package: "facebook-ios-sdk"),`
                : `// .product(name: "FacebookCore", package: "facebook-ios-sdk"),  // Disabled via config (compileOnly)`,
        },
        {
            old: new RegExp(`${swiftCommentPrefix}\\.product\\(name:\\s*"FacebookLogin",\\s*package:\\s*"facebook-ios-sdk"\\),${swiftTrailingComment}`),
            new: providerConfig.facebook === 'implementation'
                ? `.product(name: "FacebookLogin", package: "facebook-ios-sdk"),`
                : `// .product(name: "FacebookLogin", package: "facebook-ios-sdk"),  // Disabled via config (compileOnly)`,
        },
        {
            old: new RegExp(`${swiftCommentPrefix}\\.product\\(name:\\s*"GoogleSignIn",\\s*package:\\s*"GoogleSignIn-iOS"\\),${swiftTrailingComment}`),
            new: providerConfig.google === 'implementation'
                ? `.product(name: "GoogleSignIn", package: "GoogleSignIn-iOS"),`
                : `// .product(name: "GoogleSignIn", package: "GoogleSignIn-iOS"),  // Disabled via config (compileOnly)`,
        },
        {
            old: new RegExp(`${swiftCommentPrefix}\\.product\\(name:\\s*"Alamofire",\\s*package:\\s*"Alamofire"\\)${swiftTrailingComment}`),
            new: providerConfig.apple === 'implementation'
...
function configureIOS(providerConfig) {
    ...
    let success = true;
    try {
        const podspecResult = applyDependencyReplacements(fs.readFileSync(podspecPath, 'utf8'), getPodspecReplacements(providerConfig));
    ...
    try {
        const packageSwiftResult = applyDependencyReplacements(fs.readFileSync(packageSwiftPath, 'utf8'), getPackageSwiftReplacements(providerConfig));
        ...
        logError(`Error modifying Package.swift: ${error.message}`);
        success = false;
    }
    return success;
}
